...
class polygon{
	...
	//destructor
	 ...
	 void paint(){
	 	//mtr->glSetMaterial();
        // loadTexture(texFile);
        Vec2f tex[3]={Vec2f(0.0,0.0),Vec2f(1.0,0.0),Vec2f(0.5,1.0)};
        glUniform1i(glGetUniformLocation(3,"texture"),0);
	 	glBegin(GL_POLYGON);
         ...
         for(int i=0;i<vertNum;i++){
            ...
            glTexCoord2f(tex[i].x(),tex[i].y());
             ...
             //glBindAttribLocation("facenorm1");
             //glBindAttribLocation("facenorm2");
         }
	 	glEnd();
	 }
 private:
