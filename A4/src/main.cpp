...
int main(int argc, char** argv)
{
   ...
   //glewInit();
    ...
    cout<<0<<endl;
    GLuint pO=0;
  pO=  loadVertShader(vertexShaderFile);
   GLuint gO= loadGeoShader(pO,geometryShaderFile);
    GLuint fO=loadFragShader(pO,fragShaderFile);
    glLinkProgram(pO);
    GLint linked;
    ...
    if(linked){
        ...
        char* errorLog=new char[maxLength];
            glGetProgramInfoLog(pO, 1000, &maxLength, errorLog);
        printf("%s\n",errorLog);
        assert(0);
    }
    
    //Use program object
    if(pO) glUseProgram(pO);
    if(gO){
