...
CAVIGenerator::CAVIGenerator(LPCTSTR sFileName, CWnd* pView, DWORD dwRate)
...
void CAVIGenerator::SetBitmapHeader(CWnd *pView)
{
	...
	SetBitmapHeader(rect.Width(), rect.Height());
}

void CAVIGenerator::SetBitmapHeader(int width, int height)
{
...
void CAVIGenerator::MakeExtAvi()
{
	...
	// finding avi
	if( _tcsstr((const wchar_t*)m_sFile,_T("avi"))==NULL )
	{
		m_sFile+=_T(".avi");
	}
}
